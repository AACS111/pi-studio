...
test("P0-3 真并行：parallel 网关分叉的 3 个分支在时间上重叠执行（非累加串行）", async (t) => {
  ...
  // 三个并行分支各自耗时 150ms：若串行累加则 ≥450ms；真并发则三者重叠在约 150ms 窗口内。
  // DELAY 取 150（而非 80）给事件循环调度留足容差，避免 Promise.all 启动抖动导致 overlap 偶发失败。
  const DELAY = 150;
  const script = {
...
test("P0-3 嵌套并行：并行分支内部再分叉并汇聚，execution 计数与 merge 正确", async (t) => {
  ...
  const script = {
    "leader#1": { output: "开始" },
    "coder#1": { output: "拆成两块", delayMs: 40 },
    "dev1#1": { output: "开发块1完成", delayMs: 40 },
    "dev2#1": { output: "开发块2完成", delayMs: 40 },
    "tester#1": { output: "测试完成", delayMs: 40 },
    "leader#2": { output: "收尾完成，全部并行任务结束" },
  };
  const exec = createDelayedExecutor(script);
