...
test("RPC session startup opens an existing session file only once and trusts its cwd", async () => {
  ...
  // cwdOverride 重构后：open 仅出现在 hasSessionFile 分支（互斥两处各 1 次），
  // 新建会话走 create 恰好 1 次 —— 仍保证不会重复 open 同一会话文件
  assert.equal((startupSource.match(/SessionManager\.open\(/g) ?? []).length, 2);
  assert.equal((startupSource.match(/SessionManager\.create\(/g) ?? []).length, 1);
