...
/** 停止对话修复：cancel() 应把 abort 信号下发给正在执行的会话，run 快速进入 user_cancelled。
 *  回归：修复前 cancel() 仅置位 cancelled，正在执行的阻塞 executor 不会被中止（一直等到自然回合结束）。 */
test("RunManager: 取消能立即中止正在执行的角色（停止对话）", async (t) => {
  useTempDataDir(t);
  const signals = [];
  let release; // 手动放行：默认阻塞，收到 signal.abort 才返回，模拟长回合
  let startedResolve;
  const started = new Promise((r) => (startedResolve = r));
  const executor = {
    run: async (request) => {
      signals.push(request.signal);
      assert.ok(request.signal, "cancel 信号应注入每个 executor 请求");
      const sig = request.signal;
      startedResolve();
      // 阻塞直到被取消：模拟研究员/开发/文档正在跑的长回合
      await new Promise((resolve) => {
        const onAbort = () => resolve({ cancelled: true });
        if (sig.aborted) onAbort();
        else sig.addEventListener("abort", onAbort, { once: true });
      });
      return { status: "failed", output: "", failureReason: "用户取消执行" };
    },
  };
  const rm = new RunManager({ team: TEAM, runId: "run-6", executor });
  const execPromise = rm.execute(makeRun("停止测试"));
  await started; // 首个角色 leader#1 已进入 in-flight 且拿到 signal
  ...
  assert.equal(run.statusReason?.code, "user_cancelled");
  assert.equal(run.stats.agentExecutions, 1); // 只跑了被取消的这一回合，不再接力
  assert.equal(signals.length, 1);
  assert.ok(signals[0].aborted, "注入的 AbortSignal 应已被 abort");

  // 事件流终态为 run_cancelled
  const events = new EventStore(TEAM.sessionId, "run-6").replay();
  assert.equal(events[events.length - 1].type, "run_cancelled");
});

/** 停止对话修复（并行场景）：solver 模板里 研究员/开发/文档 三分支并发时，cancel 应同时中止全部 inflight。
 *  回归：修复前只有波次循环顶部的 cancelled 检查，正在并行的分支会一直跑到自然回合结束。 */
test("RunManager: 并行分支执行中取消 → 全部 inflight 被中止", async (t) => {
  useTempDataDir(t);
  const { createTeamDef: createSolver } = await import("./templates.ts");
  const solver = createSolver("sess-para", "/work", "solver-tpl", "solver");
  const started = new Set();
  let startedResolve;
  const readyPromise = new Promise((r) => (startedResolve = r));
  const gotAbort = [];
  const executor = {
    run: async (request) => {
      const sig = request.signal;
      // leader 正常完成即可路由到并行网关；只有三个分支（研究员/开发/文档）阻塞等待取消
      if (request.execution.agentId === "leader") {
        return { status: "completed", output: "任务已拆解，并行分派" };
      }
      started.add(request.execution.agentId);
      if (started.size >= 3) startedResolve(); // 三分支都进入 inflight
      await new Promise((resolve) => {
        if (sig.aborted) { gotAbort.push(request.execution.agentId); resolve({ cancelled: true }); return; }
        sig.addEventListener("abort", () => {
          gotAbort.push(request.execution.agentId);
  ...
  const rm = new RunManager({ team: solver, runId: "run-para1", executor });
  const execPromise = rm.execute(makeRun("并行停止测试"));
  await readyPromise; // 等 研究员/开发/文档 全在 inflight
  ...
  assert.equal(run.statusReason?.code, "user_cancelled");
  // 全部分支都收到 abort 信号
  assert.deepEqual(gotAbort.sort(), ["developer", "researcher", "writer"]);
  // 三个 execution 都被记录为 started，且以 failed(用户取消) 收尾，不再继续路由接力
  const events = new EventStore(solver.sessionId, "run-para1").replay();
  assert.ok(events.some((e) => e.type === "run_cancelled"));
  const startedExecs = events.filter((e) => e.type === "execution_started").length;
  // leader#1 + 三个分支 = 4 次执行已启动；取消后不再新增任何执行
  assert.equal(startedExecs, 4);
  assert.equal(run.stats.agentExecutions, 4);
});

test("RunManager: 取消兜底——executor 对 abort 无响应（prompt 卡死）也在 cancelForceMs 内强制收敛为取消", async (t) => {
  useTempDataDir(t);
  const { readRunDetail } = await import("./registry.ts");
  const team = { ...TEAM, autoSolo: false, transitions: [] };
  const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  // 模拟真实 executor 卡死在 prompt：promise 永不 resolve，也不监听信号（最坏情况）
  const stuckExecutor = {
    run: async () => {
      // 永不 resolve
  ...
  const rm = new RunManager({ team, runId: "run-cancel-force", executor: stuckExecutor, cancelForceMs: 120 });
  const p = rm.execute(makeRun("卡死场景"));
  await sleep(60);           // 让 run 启动并进入 launchAgent（executor.run 挂起）
  rm.cancel();               // 用户点停止
  const deadline = Date.now() + 600;
  let run;
  while (Date.now() < deadline) {
    run = readRunDetail(team.sessionId, "run-cancel-force").run;
    if (run.status === "cancelled") break;
    await sleep(30);
  }
  assert.equal(run.status, "cancelled", "executor 无响应时也应强制收敛为 cancelled");
  assert.equal(run.statusReason?.code, "user_cancelled");
  void p; // execute() 可能因 executor 永不返回而 pending，此处只看 run 状态
});

test("RunManager: finish 幂等——cancel 兜底与正常路径不会双收尾", async (t) => {
  useTempDataDir(t);
  const { readRunDetail } = await import("./registry.ts");
  const team = { ...TEAM, autoSolo: false, transitions: [] };
  const sleep = (ms) => new Promise((r) => setTimeout(r, ms));
  let resolveRun;
  const executor = {
    run: async ({ execution, signal }) => {
      // 既响应 abort（reject），也提供一条手动 resolve 路径
      return await new Promise((_, reject) => {
        signal?.addEventListener("abort", () => reject(new Error("用户取消执行")), { once: true });
      });
    },
  };
  const rm = new RunManager({ team, runId: "run-cancel-idem", executor, cancelForceMs: 4000 });
  const p = rm.execute(makeRun("幂等"));
  await sleep(60);
  rm.cancel();
  const deadline = Date.now() + 800;
  let run;
  while (Date.now() < deadline) {
    run = readRunDetail(team.sessionId, "run-cancel-idem").run;
    if (run.status === "cancelled") break;
    await sleep(30);
  }
  assert.equal(run.status, "cancelled");
  // 只有一次 run_cancelled 事件（finish 幂等）
  const { events } = readRunDetail(team.sessionId, "run-cancel-idem");
  const cancels = events.filter((e) => e.type === "run_cancelled");
  assert.equal(cancels.length, 1, `run_cancelled 应只出现一次，实际 ${cancels.length}`);
  void p;
});
