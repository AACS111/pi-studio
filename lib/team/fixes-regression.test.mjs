...
/* ==================== ⑦ solo 显式交接升级为编排（用户实锤场景） ==================== */

test("⑦ solo 下入口角色显式 team_handoff 交接下游 → 升级为编排实际派发（旧实现硬吞交接，下游永不执行）", async (t) => {
  useTempDataDir(t);
  const team = {
    sessionId: "sess-solo-esc", name: "solo升级", cwd: "/w", entryAgentId: "leader",
    agents: [agent("leader", "组长", "组长"), agent("fe-developer", "前端", "开发")],
    transitions: [
      { id: "t-fe-end", from: "fe-developer", to: "__end__", priority: 0, trigger: { event: "completed", condition: { mode: "always" } }, enabled: true },
    ],
    gateways: [],
    defaultRoutingMode: "strict", // strict 模式下 resolveRoute 会忽略 tool handoff —— 升级路径必须绕过它
    maxHops: 30, maxReworkRounds: 3, maxRunMinutes: 5,
    contextScope: "structured", recentCount: 20,
    createdAt: now, updatedAt: now,
  };
  ...
  const executor = {
    run: async ({ execution, onMessage }) => {
      calls.push(execution.agentId + "#" + execution.sequence);
      if (execution.agentId === "leader") {
        // 复现用户场景：leader 侦察后显式交接给下游
        return { status: "completed", output: "已交接给 fe-developer", handoffTool: { to: "fe-developer", summary: "实施 Phase 0-2" } };
      }
      onMessage({ id: `m-${execution.id}`, kind: "agent", executionId: execution.id, agentId: execution.agentId, role: execution.agentId, content: "Phase 0-2 完成", createdAt: Date.now() });
      return { status: "completed", output: "Phase 0-2 完成" };
    },
  };
  const rm = new RunManager({ team, runId: "run-solo-esc", executor });
  const finalRun = await rm.execute({ id: "run-solo-esc", teamId: "sess-solo-esc", status: "pending", complexity: "simple", task: "实现任务（分类判 simple→solo，实际需多角色）", stats: { hopCount: 0, reworkCount: 0, agentExecutions: 0, tokensUsed: 0, durationMs: 0 }, createdAt: now, updatedAt: now });
  assert.ok(calls.includes("fe-developer#1"), `下游必须被实际派发（旧实现 leader 说「已交接」但 run 以 solo 收尾），实际执行：${calls.join(",")}`);
  assert.equal(finalRun.status, "completed", `升级后 run 应正常完成，实际 ${finalRun.status}: ${finalRun.statusReason?.message ?? ""}`);
  const events = new EventStore(team.sessionId, "run-solo-esc").replay();
  const escalate = events.find((e) => e.type === "handoff_requested" && String(e.reason ?? "").includes("升级为多角色编排"));
  assert.ok(escalate, "应有 solo→编排的升级交接事件（含升级语义而非「未实际交接」）");
});
