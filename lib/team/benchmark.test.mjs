...
test("基准D 复杂可分解问题：并行+质检团队 比 串行单脑 更快且缺陷捕获率更高", async (t) => {
  ...
  assert.ok(teamRun.stats.reworkCount >= 1, "团队质检应捕获缺陷并返工");
  assert.ok(teamMs < singleMs, `团队应更快（${teamMs}ms vs ${singleMs}ms）`);
...
test("基准C 简单任务成本：autoSolo 降级到 1 次执行 vs 全链接力 N 次", async (t) => {
  useTempDataDir(t);
  // 同一“简单任务”，solo 开启 vs 关闭的执行次数对比
  const teamOn = { ...createTeamDef("sess-bm-c1", "/w", "t", "software-dev"), autoSolo: true };
  const teamOff = { ...createTeamDef("sess-bm-c2", "/w", "t", "software-dev") };

  const execOn = createMockExecutor({ "leader#1": { output: "已列目录" } });
  const on = new RunManager({ team: teamOn, runId: "run-on", executor: execOn });
  const runOn = await on.execute(makeRun(teamOn, "列出项目目录结构"));

  const execOff = createMockExecutor({
    "leader#1": { output: "拆解" },
    "product#1": { output: "方案" },
  ...
  const off = new RunManager({ team: teamOff, runId: "run-off", executor: execOff });
  const runOff = await off.execute(makeRun(teamOff, "列出项目目录结构"));

  const onExec = runOn.stats.agentExecutions;
  const offExec = runOff.stats.agentExecutions;
  console.log(`[基准C] 简单任务：solo=${onExec} 次执行，全链=${offExec} 次执行 → 成本比 ≈ ${(offExec / Math.max(onExec, 1)).toFixed(1)}x`);
  assert.equal(onExec, 1, "solo 简单任务 1 次");
  assert.ok(offExec >= 5, "全链简单任务跑遍所有角色");
});
