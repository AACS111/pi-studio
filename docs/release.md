...
- npm package: `@aacs111/pi-studio`
- GitHub Release: `AACS111/pi-studio`
...
- npm is authenticated as an account that can publish `@aacs111/pi-studio`.
...
```bash
npm view @aacs111/pi-studio@<version> version --registry https://registry.npmjs.org/
npm view @aacs111/pi-studio versions --json --registry https://registry.npmjs.org/
```
...
gh release view v<version> --repo AACS111/pi-studio
```
...
- 发布 npm 包 `@aacs111/pi-studio@<version>`。
...
- Published npm package `@aacs111/pi-studio@<version>`.
```
...
```bash
gh release edit v<version> --repo AACS111/pi-studio --notes-file - <<'EOF'
## 中文
