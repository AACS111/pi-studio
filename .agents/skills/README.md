...
| `browser-control/` | 控制右侧真实浏览器（Electron 原生 WebContentsView，或 browser-use 侧车 127.0.0.1:17865）——语义快照 snapshot + 评分定位器 + 批量执行 execute（一次 JS 上下文完成多步动作）+ select/fill/check/wait/assert + 点击/输入/滚动/导航/截图/提取页面 Markdown |
