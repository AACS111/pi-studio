...
Локальный веб-интерфейс для [pi coding agent](https://github.com/badlogic/pi-mono). Pi Studio читает локальные файлы сессий pi и предоставляет рабочее пространство в браузере для просмотра сессий, общения с агентом в реальном времени, настройки моделей, управления навыками и предварительного просмотра файлов проекта.

![Pi Studio показывает одну и ту же сессию pi рядом с CLI: структурированный Markdown, вызовы инструментов и навигация по проекту](https://raw.githubusercontent.com/AACS111/pi-studio/main/docs/screenshot2.png)

Одна и та же сессия pi в CLI и Pi Studio: структурированные вызовы инструментов, удобочитаемый Markdown, просмотр сессий и наглядные результаты.

## Быстрый старт

Для работы Pi Studio требуется Node.js **22.19.0** или новее. Проверьте версию командой `node --version`.
...
Затем откройте [http://127.0.0.1:30141](http://127.0.0.1:30141). После запуска сервера CLI попытается автоматически открыть браузер. По умолчанию Pi Studio прослушивает только `127.0.0.1`.

**Параметры:**

```bash
pi-studio --port 8080              # другой порт
pi-studio --hostname 0.0.0.0       # доступ в доверенной сети
pi-studio -p 8080 -H 0.0.0.0       # сочетание параметров
pi-studio --no-open                # не открывать браузер автоматически

PORT=8080 pi-studio                # порт также можно задать переменной окружения
PI_WEB_HOSTNAME=0.0.0.0 pi-studio  # явное разрешение сетевого доступа
PI_WEB_ALLOWED_HOSTS=pi-studio.internal pi-studio  # точное имя прокси или другого хоста
PI_WEB_PASSWORD='длинный-случайный-пароль' pi-studio  # Basic Auth (имя пользователя: pi)
PI_WEB_NO_OPEN=1 pi-studio         # удобно для фоновой службы
```
...
Pi Studio может вызывать агента с широкими полномочиями. Basic Auth не шифрует пароль при передаче, поэтому не публикуйте обычный HTTP-сервер в интернете. Для удалённого доступа используйте HTTPS через доверенный обратный прокси или надёжное VPN-подключение.
...
Для серверных запросов к моделям и API Pi Studio использует стандартные переменные окружения `HTTP_PROXY`, `HTTPS_PROXY` и `NO_PROXY`.
...
- **Каталог данных**: по умолчанию Pi Studio читает `~/.pi/agent/sessions`. Чтобы указать другой каталог агента pi, задайте `PI_CODING_AGENT_DIR`.
...
- **Git worktree**: условия отображения переключателя, создание новых рабочих деревьев и их удаление описаны в документе [Worktrees in Pi Studio](./docs/worktrees.md).
...
bin/
  pi-studio.js           # точка входа npm CLI
