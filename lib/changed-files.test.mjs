...
import { extractChangedFiles, isFilePathInsideCwd } from "./changed-files.ts";
...
// Note: the old extractGeneratedFiles() (write-kind deliverables split onto a
// separate card) was removed — generated files are now shown on the same
// unified changed-files card. write-kind rows with deliverable extensions
// still appear here, distinguished by their "A" badge and extension label.
test("deliverable extensions stay on the unified card alongside edits", () => {
  const blocks = [
    editCall("src/a.ts", "t1"),
  ...
  assert.deepEqual(extractChangedFiles(blocks, "C:/proj"), [
    ...
    { filePath: "C:/proj/script.mjs", kind: "write" },
  ]);
});
